var vm = new Vue({
  ...
  methods: {
    ...
    importData: function (data, symbol, size) {
      let names = this.namesOut(data),
        projected = this.pca.predict(data);
      data = [];
      for (let i = 0, n = projected.data.length; i < n; i++) {
        data.push([]);
        for (let j = 0; j < this.pca.U.columns; j++) {
          data[i].push(Number(projected.data[i][j].toFixed(6)))
          if (reversePC.includes(j)) {
            data[i][j] = data[i][j] * -1;
          }
        }
      }
      this.namesIn(data, names);
    ...
    savePlot: function () {
      ...
      if (this.saveHeightValid && this.saveWidthValid) {
        Plotly.downloadImage('graphDiv',{format:'png',height:this.saveHeight,width:this.saveWidth,filename:filename,scale:1});
  ...
  created: function () {
    ...
    this.options.toImageButtonOptions.filename = filename;
